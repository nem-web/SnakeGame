...
public:
    ...
    void spawnFood() {
        do {
            food.xCoord = rand() % consoleWidth;
            food.yCoord = rand() % consoleHeight;
        } while (food.xCoord < 0 || food.yCoord < 0);
    }
    ...
    void getInput() {
        if (_kbhit()) {
            ...
            if (key == 'a' || key == 'A') snake->changeDirection(DIR_LEFT);
            if (key == 's' || key == 'S') snake->changeDirection(DIR_DOWN);
            if (key == 'd' || key == 'D') snake->changeDirection(DIR_RIGHT);
        }
    }
};
